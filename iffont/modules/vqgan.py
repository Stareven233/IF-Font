...
from modules.diffusionmodules import Encoder, Decoder
from modules.quantize import VectorQuantizer2 as VectorQuantizer
from modules.quantize import GumbelQuantize
from modules.quantize import EMAVectorQuantizer
...
class VQModel(pl.LightningModule):
  ...
  def __init__(
    ...
    if not lossconfig.target.endswith('DummyLoss'):
      print('warning: redundant loss function in VQModel')
    self.loss = DummyLoss()
  ...
  def encode(self, x):
    h = self.encoder(x)
    h = self.quant_conv(h)  # z_e: h.shape=torch.Size([1, 4, 32, 32])
    quant, emb_loss, info = self.quantize(h)  # z_q: quant.shape=torch.Size([1, 4, 32, 32])
  ...
  def configure_optimizers(self):
    ...
    opt_ae = torch.optim.Adam(list(self.encoder.parameters()) + list(self.decoder.parameters()) + list(self.quantize.parameters()) + list(self.quant_conv.parameters()) + list(self.post_quant_conv.parameters()),
                              lr=lr,
                              betas=(0.5, 0.9))
...
class VQSegmentationModel(VQModel):
  ...
  def validation_step(self, batch, batch_idx):
    ...
    self.log("val/total_loss", total_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
    return aeloss
...
class VQNoDiscModel(VQModel):
  ...
  def configure_optimizers(self):
    optimizer = torch.optim.Adam(list(self.encoder.parameters()) + list(self.decoder.parameters()) + list(self.quantize.parameters()) + list(self.quant_conv.parameters()) + list(self.post_quant_conv.parameters()),
                                 lr=self.learning_rate,
                                 betas=(0.5, 0.9))
    return optimizer


# 不能用，得准备temperature_scheduler
...
class EMAVQ(VQModel):
  ...
  def __init__(
    ...
    self.quantize = EMAVectorQuantizer(n_embed=n_embed, embedding_dim=embed_dim, beta=0.25, remap=remap)
